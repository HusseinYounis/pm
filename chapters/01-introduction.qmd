...
## What Is a Project? [📋 Required]{.badge-req}
...
## The Project Lifecycle [📋 Required]{.badge-req}
...
## The Triple Constraint [📋 Required]{.badge-req}
...
## The PMBOK Knowledge Areas [📋 Required]{.badge-req}
...
## Why Projects Fail [📋 Required]{.badge-req}
...
## How IT Projects Differ from Other Projects [➕ Extended]{.badge-ext}

This book is based on **Schwalbe's IT Project Management** framework, which recognises that IT projects have unique characteristics that make them fundamentally different from construction, manufacturing, or infrastructure projects. Understanding these differences explains why IT projects fail more often — and why Agile methods emerged specifically for them.

```{mermaid}
%%| fig-cap: "Key differences between IT projects and other project types"
flowchart LR
    subgraph IT["💻 IT Projects"]
        IT1["Invisible deliverable\n(software, data, systems)"]
        IT2["Requirements change\nfrequently"]
        IT3["Progress hard\nto measure"]
        IT4["Cheap to rework\nbefore release"]
        IT5["High technology\nuncertainty"]
    end
    subgraph OTHER["🏗️ Engineering / Construction Projects"]
        O1["Visible, physical\ndeliverable"]
        O2["Scope changes\nare costly & rare"]
        O3["Progress visible\n(% built)"]
        O4["Rework is\nextremely expensive"]
        O5["Established methods\nand standards"]
    end

    style IT fill:#dbeafe,stroke:#3b82f6
    style OTHER fill:#dcfce7,stroke:#22c55e
```

| Characteristic | IT Projects | Construction / Engineering |
|---|---|---|
| **Deliverable** | Intangible — software, database, system | Tangible — building, road, plant |
| **Visibility of progress** | Difficult — "90% done" is unreliable | Easy — you can see what is built |
| **Requirement changes** | Frequent and expected | Rare and expensive |
| **Rework cost** | Low before release; very high after | Always very high |
| **Technology risk** | High — tools and platforms evolve rapidly | Low — established construction methods |
| **Uniqueness** | Almost every project is one-of-a-kind | Some reuse of standard designs |
| **Team skills** | Highly specialised, scarce, hard to replace | Broader labour pools available |
| **Failure rate** | ~52% challenged, ~17% fail (Standish 2020) | Lower, but cost overruns are common |

: IT vs. Engineering project characteristics {.striped .hover}

::: {.callout-note icon="false"}
## 📖 Why This Matters for Project Management

These differences explain three important things:

1. **Why Agile was invented for IT** — Fixed-scope waterfall planning fails when requirements change every sprint. IT projects need iterative, flexible approaches.
2. **Why IT projects are harder to control** — You cannot walk the site and see progress. Earned Value, burndown charts, and demos are used instead.
3. **Why stakeholder involvement is critical in IT** — Since the product is invisible until it is built, continuous user feedback is the only way to ensure the final system meets the real need.
:::

---

## The Role of the Project Manager [📋 Required]{.badge-req}
...
## Organisational Structures for Projects [📋 Required]{.badge-req}
...
::: {.callout-note icon="false"}
## 📖 Key Roles — PM vs. Functional Manager (FM)

| Role | Who They Are | What They Control |
|---|---|---|
| **Project Manager (PM)** | Leads the project — responsible for scope, schedule, cost, and quality | Project deliverables and outcomes |
| **Functional Manager (FM)** | Head of a department (e.g., Head of Engineering, Head of Finance, Head of HR) | People, salaries, technical standards, and departmental resources |

The **central question** in every organisational structure is: *between the PM and the FM — who has more power?*
:::
...
In a **divisional structure**, the organisation is split into self-contained units — each division has its **own functional departments** (Engineering, Finance, HR, etc.) and operates semi-independently. Divisions are typically organised by:

- **Product** (e.g., Infrastructure Division, Buildings Division, Energy Division)
- **Geography** (e.g., Middle East Division, Europe Division)
- **Customer/Market** (e.g., Government Clients Division, Private Sector Division)

```{mermaid}
%%| fig-cap: "Divisional organisational structure — each division is a mini-organisation"
flowchart TD
    CEO["🏢 CEO / Head Office"]
    CEO --> D1["🏗️ Infrastructure\nDivision"]
    CEO --> D2["🏢 Buildings\nDivision"]
    CEO --> D3["⚡ Energy\nDivision"]
    ...
    style D3 fill:#b45309,color:#fff
```

::: {.callout-note icon="false"}
## ⚖️ PM Authority: **Medium — Authority Within the Division**

The PM operates within the division and reports to the Division Head. Authority is stronger than in a functional structure but limited to the division's resources — cross-division collaboration requires executive-level approval.
:::

| | Divisional |
|---|---|
| **PM Authority** | Medium (within division) |
| **Resource Control** | Division manager |
| **PM Role** | Full-time within division |
| **Budget Control** | Division manager |
| **Best For** | Large organisations with distinct product lines or regional operations |

**✅ Advantages**
- Each division is focused and accountable for its own results
- Faster decision-making within the division
- Clear ownership of divisional projects and budgets
- Easier to measure divisional performance

**❌ Disadvantages**
- Duplication of functions across divisions (each has its own HR, Finance, etc.)
- Poor collaboration and resource sharing between divisions
- Potential for divisions to compete against each other internally
- Inconsistent PM practices across divisions unless a central PMO exists
...
| Characteristic | Functional | Divisional | Weak Matrix | Balanced Matrix | Strong Matrix | Projectised |
|---|---|---|---|---|---|---|
| **PM Authority** | None | Medium | Low | Medium | High | Full |
| **Resource Availability** | Low | Medium | Low | Medium | High | Full |
| **Budget Control** | Functional Mgr | Division Mgr | Functional Mgr | Mixed | PM | PM |
| **PM Role** | Part-time | Full-time | Part-time | Full-time | Full-time | Full-time |
| **Team Loyalty** | Department | Division | Department | Shared | Project | Project |
| **Decision Speed** | Slow | Moderate | Slow | Moderate | Fast | Fastest |
